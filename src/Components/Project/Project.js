...
const Project = () => {
    return (
        ...
        <div className="container  md:flex item-center justify-center my-4 mx-8">
            {
                project.map(project=>
                    <div key={project.id} className=" h-full mx-auto  md:m-4 grid-cols-1 grid-rows-3 my-4 rounded-md w-80 shadow /*hover:shadow-xl hover: transition duration-500 ease-in-out hover:  text-3xl transform hover:-translate-y-1 hover:scale-110*/">
                ...
                <div className="">
                  ...
                  <button className=" text-white bg-red-500 h-10 w-1/3 text-align-center text-xs rounded-md mb-4 border "><a className='text-white no-underline' target="_blank" rel="noreferrer" href={project.code}>github </a></button>
                </div>
            </div>)
            }
